<template>
		...
		>
			...
			<ul class="flex gap-1 justify-center mt-[60px]" v-if="slides && slides.testimonials">
				<li
					v-for="(item, index) in slides.testimonials"
					:class="['w-[12px] h-[12px] rounded-full bg-gray-soft in-fast duration-300', currentIndex === index ? 'slide--active': '']"
				></li>
			</ul>
		</div>
...
const { data: slides } = await useFetch(
	'https://webnights.github.io/apis/HomePro/testimonials.json'
)
</script>
